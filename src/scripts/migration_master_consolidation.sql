-- ============================================================
-- ENERMASS Solar Calculator — Migration: Master Pricing Consolidation
-- ============================================================

-- 1. Drop rate_master overrides table, triggers, and policies
DROP TABLE IF EXISTS rate_master CASCADE;

-- 2. Modify eq_panels and migrate pricing data
...
-- Drop generated column and rate_per_watt column
ALTER TABLE eq_panels DROP COLUMN IF EXISTS rate_per_panel CASCADE;
ALTER TABLE eq_panels DROP COLUMN IF EXISTS rate_per_watt CASCADE;

-- Enforce NOT NULL on panels pricing columns
ALTER TABLE eq_panels ALTER COLUMN buy_price SET NOT NULL;
ALTER TABLE eq_panels ALTER COLUMN selling_price SET NOT NULL;


-- 3. Modify other equipment tables
DO $$
BEGIN
    -- eq_inverters
...
-- 4. Recreate active equipment views
DROP VIEW IF EXISTS v_active_panels CASCADE;
...
DROP VIEW IF EXISTS v_active_inverters CASCADE;
...
DROP VIEW IF EXISTS v_active_batteries AS
...
-- 5. Recreate mounting structures get_structure_rate function
