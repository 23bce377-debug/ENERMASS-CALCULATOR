...
async function main() {
  ...
  for (const org of toDelete) {
    try {
      ...
      // Let's delete from child tables manually to avoid FK constraint issues since they might not be ON DELETE CASCADE.
      ...
      console.error(`Failed to delete ${org.name}:`, err.message);
    }
  }
